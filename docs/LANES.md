...
- **descendants-all** (2026-08-25): no prior idea-descendant extraction
  ...
  - Result: **30 genuine descendant repos located across 17 of 123
    papers** (4 mechanical + 26 verified-search), written to
    `harvest/repos/descendants.json`. Every unlocated row is honestly
    `located: false` ("paper-only"), never guessed.
  - **Not yet done**: the "widened" `uses-tool`/core wave (1,060 rows,
    the larger, lower-per-row-value bucket from the sketch-frontend
    widening) only got the free mechanical scan -- the paid search+verify
    step would cost roughly proportionally more (~$0.15-0.30 in model
    verification calls at the same confirm rate, plus ~40 min of GitHub
    search-API-limited wall time for ~1,060 more searches). Resumable via
    `python3 curate/build_idea_descendants.py --search --wave widened`
    then `curate/verify_descendants.py --write` whenever that's wanted.
    (A background run of this exact command is in flight as of 2026-08-25
    -- a worker session claiming `own-repo-deep-hunt` below saw the live
    process and `harvest/repos/descendants.json` uncommitted mid-run;
    left it alone, did not touch that file.)
