...
| **ui-tests** / **Lane A (UI TESTS)** | `tests/ui/**` | **active** — round-11 task 1 (old spec): 908 cases, 907 passed, 1 bug found, closed green by Fable (task 2, `cf72756e`). Round-11 task 1 (new spec, `tests/ui/SPEC.md`, 2026-08-26): `tests/ui/random_settings_test.py`, **RUN COMPLETE — 100 tests, 100 passed, 0 failed, seed 42**. Re-claimed as round-12 Lane A (2026-08-26) under the new disjoint-path lane model -- same claimed paths (`tests/ui/**`), re-running both harnesses against Lane E's subsequent UI commits per the pre-cutover-gate charter -- see lane log + `tests/ui/report.md`. |
...
- **UI combinatorial test run** (round 11 task 1). `tests/ui/oracle.py`:
  an independent Python re-implementation of `assets/js/publications.js`'s
  dedupe (title+type, url>doi>slides tiebreak), facet/filter logic
  (`tagsOf`/`topicsOf`/`projectsOf`, `listNormalizedAuthors`,
  `categoryKeyOf`/`venueShortOf` incl. venue-mode), `filteredItems`
  (AND-across-facets/OR-within-facet, title search, minCites/minImpact
  thresholds), and `renderList`'s grouping + multi-key tiebreak sort —
  read the real JS in full (twice, since the file is under heavy
  concurrent edit from other lanes and had grown past what I'd read
  earlier) rather than guess, to keep the oracle byte-for-byte faithful.
  Also reimplements the composite-impact/quantile-tier machinery from
  `assets/js/citations.js` (`impactScore`/`WEIGHTS`/quantile thresholds)
  since the Impact sort mode and slider depend on it.
  `tests/ui/facet_test.py`: Playwright driver against the already-running
  local server (`:8123`, repo root) — every single value of Years/Topics/
  Projects/Authors/Type-mode-Categories/Venue-mode-Categories (586 cases),
  a 30-name stratified sample of "Cited and Used by" (6,215 values —
  exhaustive impractical, disclosed in the report as a scope note, not a
  silent cap), 200 sampled pairwise combos, 100 random 3-4-way combos, and
  a 15-case sample additionally exercised across 5 Group&sort presets plus
  a smoke pass over the citation panel's global Impact/Recency/Popularity
  sort and the three Show-summaries/citations/repositories toggles
  (paper-count-unchanged + zero-console-errors check, since those don't
  change list membership). Matching is by rendered title-text sequence
  (order-sensitive) against the oracle's predicted membership+order per
  group header — no DOM identity hook was added to the site for this, to
  keep the change footprint at zero in a file several other lanes are
  actively editing.
  Result: **908 real cases, 907 passed**, `tests/ui/report.md`.
- **Found a real structural bug** (STOP for Fable, not fixed here — out
  of the display-only-fix bound the task set): after **any** click of
  "Clear filters", the Years / Topics & Projects / Categories checkboxes
  silently stop filtering for the rest of the page session — a click
  visually snaps the checkbox back to unchecked and the paper count never
  moves. Authors and "Cited and Used by" are unaffected. Root cause:
  `clearAll()` (`assets/js/publications.js` ~line 1508) reassigns
  `state.years`/`state.keywords`/`state.types` to brand-new `{}` objects,
  but only `rebuildAuthorFacet()`/`rebuildCiteAuthorFacet()` rebuild their
  facet boxes afterward — the other three boxes' checkbox `onchange`
  closures keep writing into the *discarded* old object, so
  `updateFacetCounts()` (reading the real, still-empty `state.<facet>`)
  snaps the checkbox back off on the very same click. Fix sketch in the
  report: either rebuild all five facet boxes in `clearAll()`, or delete
  keys in place instead of reassigning the state objects. This is a
  regression the site-citations lane's own earlier browser verification
  didn't catch because that pass tested Clear as a terminal reset, not a
  before-a-later-selection step — noting here so nobody re-litigates
  whether that earlier "zero console errors" verification and this
  finding conflict; they don't, they tested different things.
  This one failure is filed as its own dedicated repro case
  (`run_clear_filters_bug_case`) rather than left to surface as noise
  among the single-facet cases, precisely so task 2 (Fable) can jump
  straight to it.
- Harness is a real regression gate going forward — cheap to rerun
  (`python3 tests/ui/facet_test.py`, ~2-3 min headless) against any future
  `assets/js/publications.js`/`citations.js` change; recommend wiring
  into `docs/refresh.md`'s pre-cutover checklist per task 2's own note.
- **UI random-settings test run, new spec** (round 11 task 1 re-posted
  with `tests/ui/SPEC.md` — his own instruction, supersedes the terse
  original text). `tests/ui/random_settings_test.py`: discovers every
  interactive control inside `#pubs-filters` **at runtime** (a
  `page.evaluate` walk of every `input`/`select`/`button`, reduced
  programmatically into checkbox-facets/toggle-groups/selects/ranges/
  text-inputs/radios/lone-buttons — nothing hardcoded, so a control this
  spec never anticipated still gets bucketed and exercised) and writes
  the inventory into the report. Reuses `tests/ui/oracle.py` unchanged
  and imports `facet_test.py` for the Driver base + a couple of helpers
  rather than duplicating them. 100 settings: 25 single-control, 60
  multi (2-4 controls, sometimes with a Group&sort change folded in),
  15 "extremes" (kitchen-sink — one value from each of the four main
  facets simultaneously, since checking *every* value within one facet
  is a no-op under OR-within-facet semantics; a search-derived
  guaranteed-empty year+author pair; both threshold sliders pinned to
  max; a title search guaranteed to match nothing). 30 of the 100 are
  flagged for the spec's expansion checks (Summary/Repositories/
  Citations on 1-3 papers each); 19 actually expanded one (the other 11
  landed on an empty-result setting — disclosed in the report, not a
  silent skip) — verifies lazy-load-on-expand via a real network-request
  log (a paper's `data/citations/<key>.json` must not appear before its
  toggle is clicked, must appear after, and must not refetch across an
  expand/collapse/expand/collapse cycle), head-count text against the
  data file, absence of `undefined`/`null`/`NaN`/raw markup in an opened
  Summary, that a paper absent from an index gets no toggle promising
  data it doesn't have, and zero console errors / zero failed or 4xx+
  requests throughout. Two explicit STOP-rather-than-guess calls, stated
  in the report rather than silently assumed: (1) this codebase has no
  second "All Papers panel" distinct from the "Publications" listing —
  both his terms refer to the one page's two regions (`#pubs-filters`
  and `#pubs-results`), so "cross-page agreement" is read as "a facet's
  dynamic count badge must match what's actually rendered below it";
  (2) "every filter on at once" is read as the kitchen-sink combo above,
  not literally every checkbox checked (which is a no-op). Found and
  fixed three of my own harness bugs before trusting a green run: (a)
  network-URL matching needs `encodeURIComponent`-equivalent escaping —
  a bibtexKey's `:` becomes `%3A` in the real fetch URL, so a raw
  substring check against the unescaped key silently never matches;
  (b) expansion-candidate identity must come from the oracle's item
  objects, not from rendered title text — two distinct records (e.g. a
  thesis and its sibling paper) can share an identical title post-dedup,
  and sampling from title strings can silently collapse two picks onto
  one item; (c) a paper's Citations and Repositories panels both use the
  shared `.cite-head-count` class, so reading "the first one in the
  `<li>`" silently reads the Citations panel's count even when checking
  Repositories — fixed by indexing `.cite-view` by attach order instead.
  **RUN COMPLETE — 100 tests, 100 passed, 0 failed, seed 42.**
  `docs/refresh.md`'s pre-cutover gate section updated to run both
  harnesses.
